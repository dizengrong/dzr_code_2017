...
class Player(wx.Frame):
    ...
    def __init__(self, title):
        ...
        self.videopanel2.SetBackgroundColour(wx.WHITE)
        ...
        self.m_staticText = wx.StaticText( self.videopanel, wx.ID_ANY, "fps:")
        self.m_staticText.SetForegroundColour(wx.RED)
        self.m_staticText2 = wx.StaticText( self.videopanel2, wx.ID_ANY, "fps:")
        self.count = 0
    ...
    def OnExit(self, evt):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            file = dlg.GetPath()
            self.Media2 = self.Instance.media_new(file)
            self.player2.set_media(self.Media2)
            handle = self.videopanel2.GetHandle()
            self.player2.set_hwnd(handle)
            self.player2.play()
            # self.timer.Start()
        dlg.Destroy()
    ...
    def OnTimer(self, evt):
        ...
        self.count += 1
        if self.count % 25 == 0:
            self.m_staticText.SetLabel("fps:%s" % (self.player.get_fps()))
