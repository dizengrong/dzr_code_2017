...
def do_pull(repo_conf):
	...
	print("update to: %s %s" % (ret.sha().hexdigest(), ret.message.decode("UTF-8").strip())) 
	print("at branch %s commit time: %s" % (branch_name, time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(ret.commit_time))))
	print("\n")
