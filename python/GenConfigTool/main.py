...
class MyFrame1 (wx.Frame):
    ...
    def __init__(self, parent):
        ...
        self.m_searchCtrl1 = wx.SearchCtrl(
    ...
    def OnCellRightClick(self, event):
        ...
        self.sell_tab_clicked_col = event.GetCol()
        self.PopupMenu(menu)

        menu.Destroy()
        self.sell_tab_clicked_row = None
        self.sell_tab_clicked_col = None
    ...
    def OnOpenFileInExplore(self, event):
        if self.sell_tab_clicked_row is not None:
            row = self.sell_tab_clicked_row
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            for tpl_dict in tpl_dicts:
                ...
                try:
                    ...
                    msg_dlg = wx.lib.dialogs.ScrolledMessageDialog(
                        self, msg, u"导出失败")
                    msg_dlg.ShowModal()
                    return
            msg = u"成功导出的文件列表:\n    " + succ_files + "\n"
    ...
    def DoExport(self, tpl_dict, dest_dir):
        ...
        for data in tpl_dict['datas']:
            excle_filename = os.path.join(
                self.excle_src_path, excle_file.encode("GBK"))
            ...
            if sort_col is '':
                ...
                dict[key].sort(cmp=lambda x, y: cmp(
                    x[sort_col], y[sort_col]), reverse=True)
        # render template with dict data
        content = engine.render(os.path.join(
            self.cwd, 'config'.encode("GBK"), tpl.encode("GBK")), dict)
