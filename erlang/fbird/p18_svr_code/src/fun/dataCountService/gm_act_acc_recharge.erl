...
send_info_to_client(Uid, Sid, ActivityRec, UsrActivityRec) ->
	Pt = #pt_acc_recharge_info{
		...
		picture   = ActivityRec#gm_activity.picture,
		datas     = get_reward_state_list(Uid, ActivityRec, UsrActivityRec)
	},
	?send(Sid, proto:pack(Pt)).

%% 领取奖励的item_way日志
get_reward_way() -> ?ITEM_WAY_GM_ACT_ACC_RECHARGE.
...
do_activity_end_help2(Uid, ActivityRec, PtState) ->
	#pt_public_acc_recharge_des{state = State, products = RewardItems} = PtState,
	case State of
		?REWARD_STATE_CAN_FETCH ->
			fun_gm_activity_ex:send_not_fetch_mail(?GM_ACTIVITY_ACC_RECHARGE, Uid, ActivityRec#gm_activity.act_name, RewardItems, 1);
		_ -> skip
	end.	
...
test_set_config() ->
	ActivityRec = #gm_activity{
		act_id       = ?GM_ACTIVITY_ACC_RECHARGE,
		act_name     = "name",
		type         = ?GM_ACTIVITY_ACC_RECHARGE,
		start_time   = util:unixtime() + 10,
		end_time     = util:unixtime() + 20000,
		act_des      = "ActDes",
		setting      = [],
		reward_datas = util:term_to_string(test_reward_datas(?GM_ACTIVITY_ACC_RECHARGE))
	},
...
test_del_config() ->
	fun_gm_activity_ex:del_config(?GM_ACTIVITY_ACC_RECHARGE, ?GM_ACTIVITY_ACC_RECHARGE).

test_reward_datas(?GM_ACTIVITY_ACC_RECHARGE) ->
	[{100, [{110062,1}], 1, "SortDescripte", "ActivityDescripte"},
	 {300, [{110061,1}], 1, "SortDescripte", "ActivityDescripte"},
	 {500, [{110060,1}], 1, "SortDescripte", "ActivityDescripte"},
	 {980, [{110062,1}], 2, "SortDescripte", "ActivityDescripte"},
	 {1980, [{110061,1}], 2, "SortDescripte", "ActivityDescripte"},
	 {3280, [{110060,1}], 2, "SortDescripte", "ActivityDescripte"},
	 {6480, [{110062,1}], 3, "SortDescripte", "ActivityDescripte"},
	 {8888, [{110061,1}], 3, "SortDescripte", "ActivityDescripte"},
	 {9999, [{110060,1}], 3, "SortDescripte", "ActivityDescripte"}].
