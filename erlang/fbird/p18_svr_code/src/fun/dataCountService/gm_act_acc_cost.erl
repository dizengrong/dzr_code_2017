...
%% 领取奖励的item_way日志
get_reward_way() -> ?ITEM_WAY_GM_ACT_ACC_COST.
...
check_fetch_reward(Uid, UsrActivityRec, ActivityRec, RewardId) ->
	...
	case lists:keyfind(RewardId, #pt_public_acc_cost_des.id, StateList) of
		false -> {error, "error_fetch_reward_not_reached"};
		#pt_public_acc_cost_des{state = ?REWARD_STATE_CAN_FETCH, items = RewardItems} ->
