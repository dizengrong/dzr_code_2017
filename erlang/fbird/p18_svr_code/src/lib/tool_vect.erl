...
-export([add/2,dec/2,ride/2,dot_ride/2,cross_ride/2,lenght/1,lenght_power/1,normal/1]).
-export([check_line_and_face/5,check_point_in_face/4,get_cross_point/4,check_point_on_line/3]).
...
cross_ride(#map_point{x = X1,y = Y1,z = Z1},#map_point{x = X2,y = Y2,z = Z2}) -> #map_point{x = Y1 * Z2 - Z1 * Y2,
																							y = Z1 * X2 - X1 * Z2,
																							z = X1 * Y2 - Y1 * X2}.
lenght(#map_point{x = X,y = Y,z = Z})-> math:sqrt(X*X + Y*Y + Z*Z).
lenght_power(#map_point{x = X,y = Y,z = Z})-> X*X + Y*Y + Z*Z.
normal(Point) -> ride(Point,1/lenght(Point)).

%%计算线段交点
get_cross_point(A,B,C,D) ->
	Denominator = (B#map_point.z - A#map_point.z) * (D#map_point.x - C#map_point.x) - (A#map_point.x - B#map_point.x) * (C#map_point.z - D#map_point.z),
%% 	io:format("Denominator = ~p~n",[Denominator]),
	if
		Denominator == 0 -> 
			%%平行或共线
			no_cross;
		true ->
			X = ((B#map_point.x - A#map_point.x) * (D#map_point.x - C#map_point.x) * (C#map_point.z - A#map_point.z)
				+(B#map_point.z - A#map_point.z) * (D#map_point.x - C#map_point.x) * A#map_point.x
				-(D#map_point.z - C#map_point.z) * (B#map_point.x - A#map_point.x) * C#map_point.x ) / Denominator,
			Z = -1 * ((B#map_point.z - A#map_point.z) * (D#map_point.z - C#map_point.z) * (C#map_point.x - A#map_point.x)
				+(B#map_point.x - A#map_point.x) * (D#map_point.z - C#map_point.z) * A#map_point.z
				-(D#map_point.x - C#map_point.x) * (B#map_point.z - A#map_point.z) * C#map_point.z ) / Denominator,
%% 			io:format("X,Z = ~p~n",[{X,Z}]),
			Point = #map_point{x = X,y = 0,z = Z},
			%%交点是否在线段上
			CrossAB = check_point_on_line(A,B,Point),
			CrossCD = check_point_on_line(C,D,Point),
			if
				CrossAB andalso CrossCD -> {ok,Point};
				true -> no_cross
			end
	end.

check_point_on_line(A,B,P) ->
	if
		(P#map_point.x - A#map_point.x) * (P#map_point.x - B#map_point.x) =< 0 andalso (P#map_point.z - A#map_point.z) * (P#map_point.z - B#map_point.z) =< 0 -> true;
		true -> false
	end.
...
check_line_and_face(LinePoint1,LinePoint2,FacePoint1,FacePoint2,FacePoint3) ->
	...
	if
		...
		true ->
			...
			{ok,Dot,add(LinePoint1,ride(U,T))}
	end.
...
get_radian(V1,V2) ->
	...
	if
		Null1 == false andalso Null2 == false -> 
			R = dot_ride(V1,V2) / (lenght(V1) * lenght(V2)),
			RR = if
					 R > 1 -> 1;
					 R < -1 -> -1;
					 true -> R
...
dot_line_dis(#map_point{x = LX,z = LZ},#map_point{x = PX,z = PZ}) -> 
	try
		util:abs((LZ / LX) * PX - PZ) / lenght(#map_point{x = (LZ / LX),y = 0,z = 1})
...
get_distance({X,Z},{X1,Z1})->
	Data=(X-X1)*(X-X1)+(Z-Z1)*(Z-Z1),
	math:sqrt(Data).
