...
-export([check_point/1,check_dir/3,check_dir_by_point/2]).
-export([set_module/1,get_point/1,get_line/1,get_face/1,get_divide_info/0,get_divide/1]).

set_module(Name) -> put(map_moudle,Name).

get_point(Index) -> 
	Module = get(map_moudle),
	Module:get_point(Index).
get_line(Index) -> 
	Module = get(map_moudle),
	Module:get_line(Index).
get_face(Index) -> 
	Module = get(map_moudle),
	Module:get_face(Index).
get_divide_info() -> 
	Module = get(map_moudle),
	Module:get_divide_info().
get_divide(Index) -> 
	Module = get(map_moudle),
	Module:get_divide(Index).

%% 先找到该点所在的区域，然后遍历区域内的所有面，对于一个面找到该点与这个面的交点。然后看这个交点是否在这个面上？
check_point(Point) ->
	...
	case get_divide(DivideIndex) of
		no -> false;
		FaceIndexs ->
			% ?debug("Point = ~w, FaceIndexs = ~w",[Point, FaceIndexs]),
...
check_dir_by_point(Point,ToPoint) when is_record(Point,map_point) andalso is_record(ToPoint,map_point) ->
	...
	case tool_vect:is_null(Dir) of
		true -> {error,no_dis};
		_ ->			
			check_dir_by_point(Point,Dir,ToPoint,no,Point,tool_vect:lenght(tool_vect:dec(ToPoint#map_point{y=0}, Point#map_point{y=0})))
	end;
...
check_dir_by_point(Point,Dir,ToPoint,FromFace,FromPoint,NeedDis) ->
	...
	case find_dir_point(FromPoint,Dir,FromFace) of
		{CanCantinue,NextFace,NextPoint} ->
			% ?debug("CanCantinue,FromFace,NextFace,NextPoint = ~w",[{CanCantinue,FromFace,NextFace,NextPoint}]),
			ThisDis = tool_vect:lenght(tool_vect:dec(NextPoint#map_point{y = 0},Point#map_point{y = 0})),
			% ?debug("ThisDis = ~p, NeedDis = ~p",[ThisDis, NeedDis]),
			if
				...
				true ->
					case CanCantinue of
						true -> 
							% ?debug("continue to NextFace:~w", [NextFace]),
							check_dir_by_point(Point,Dir,ToPoint,NextFace,NextPoint,NeedDis);
...
check_dir(Point,Dir,MaxDis,FromFace,FromPoint) ->
	% ?debug("FromPoint:~w, Dir:~w, FromFace:~w",[FromPoint, Dir, FromFace]),
	case find_dir_point(FromPoint,Dir,FromFace) of
		{CanCantinue,NextFace,NextPoint} ->
			...
			if
				ThisDis >= MaxDis -0.01 ->
					NoYMaxDisPoint = tool_vect:add(Point#map_point{y = 0}, tool_vect:ride(tool_vect:normal(Dir#map_point{y = 0}), MaxDis)),
...
check_in_polygon(Point,FaceIndex) ->
	case check_line_and_face(Point,tool_vect:add(Point,#map_point{x = 0 , y = 1 , z = 0}),FaceIndex) of
		{ok,_,CrossPoint} -> 
			% ?DEBUG("check_in_polygon:~p", [CrossPoint]),
			case check_point_in_face(CrossPoint,FaceIndex) of
				true -> 
					% ?DEBUG("find cross point:~p", [CrossPoint]),
					{true,CrossPoint};
				_ -> 
					% ?DEBUG("not find cross point", []),
					false
			end;
		_ ->
			% ?DEBUG("no cross point"), 
			false
	end.
...
%% lib_map_module:check_point({map_point,101.90274497588149,0.0,173.79469344973262}).  --> 568
%% lib_map_module:check_point({map_point,101.52150268160915,0.0,174.18753797831195}).  --> 569
%% lib_map_module:check_point({map_point,101.1551790904388,0.0,174.56500978729753}).  --> 577
% 568 --> 569 --> 577
...
%% 							?debug("get_face cannot find why?"),
				...
				_ -> 
					...
					case check_point(FromPoint) of
						{true,CheckFromFace,CheckCrossPoint} -> 
					% ?debug("============FromPoint:~w", [FromPoint]),
							{false,CheckFromFace,CheckCrossPoint};
						R -> 
							%%还不能走
					% ?debug("============"),
