...
-export([
    ...
    %% QLC functions
    table/1, table/2, size/1,
    dirty_foldl/3, dirty_map/2, dirty_foldl_limit/4, dirty_map_limit/3,
    dirty_map_from_end/2, dirty_map_limit_from_end/3
]).
...
delete_sure(Tab, Key, LockKind) ->
    case mnesia:read(Tab,Key) of
        []->
            db_api:abort(key_not_found);
        _ ->
            db_api:delete(Tab, Key, LockKind)
    end.
...
delete_object_sure(Tab, Record, LockKind) ->
    case db_api:match_object(Tab, Record, write) of
        []->
            db_api:abort(object_not_found);
        _->
            db_api:delete_object(Tab, Record, LockKind)
    end.
...
dirty_delete_all_object(Tab) ->
    [dirty_delete(Tab, Key) || Key <- dirty_all_keys(Tab)].
...
%% 脏foldl遍历Tab，Fun(Rec, Acc) -> Acc2. 返回遍历的结果Acc2
dirty_foldl(Fun, Acc, Tab) -> 
    dirty_foldl_help(Fun, Acc, Tab, dirty_first(Tab)).

dirty_foldl_help(_Fun, Acc, _Tab, '$end_of_table') ->
    Acc; 
dirty_foldl_help(Fun, Acc, Tab, Key) -> 
    Acc2 = Fun(hd(dirty_read(Tab, Key)), Acc),
    dirty_foldl_help(Fun, Acc2, Tab, dirty_next(Tab, Key)).


%% 与dirty_foldl/3同，只是遍历前N个记录
dirty_foldl_limit(Fun, Acc, Tab, N) -> 
    dirty_foldl_limit_help(Fun, Acc, Tab, N, 1, dirty_first(Tab)).

dirty_foldl_limit_help(_Fun, Acc, _Tab, _N, _Count, '$end_of_table') ->
    Acc; 
dirty_foldl_limit_help(_Fun, Acc, _Tab, N, Count, _Key) when Count > N -> 
    Acc; 
dirty_foldl_limit_help(Fun, Acc, Tab, N, Count, Key) -> 
    Acc2 = Fun(hd(dirty_read(Tab, Key)), Acc),
    dirty_foldl_limit_help(Fun, Acc2, Tab, N, Count + 1, dirty_next(Tab, Key)).


%% 脏遍历Tab，Fun(Rec) -> Ret. 返回遍历的结果[Ret]
dirty_map(Fun, Tab) -> 
    dirty_map_help(Fun, [], Tab, dirty_first(Tab)).

dirty_map_help(_Fun, Acc, _Tab, '$end_of_table') ->
    lists:reverse(Acc); 
dirty_map_help(Fun, Acc, Tab, Key) -> 
    Acc2 = [Fun(hd(dirty_read(Tab, Key))) | Acc],
    dirty_map_help(Fun, Acc2, Tab, dirty_next(Tab, Key)).



%% 同dirty_map/2，不过是从表的末尾开始往前读
dirty_map_from_end(Fun, Tab) -> 
    dirty_map_from_end_help(Fun, [], Tab, dirty_last(Tab)).

dirty_map_from_end_help(_Fun, Acc, _Tab, '$end_of_table') ->
    lists:reverse(Acc); 
dirty_map_from_end_help(Fun, Acc, Tab, Key) -> 
    Acc2 = [Fun(hd(dirty_read(Tab, Key))) | Acc],
    dirty_map_from_end_help(Fun, Acc2, Tab, dirty_prev(Tab, Key)).


%% 与dirty_map/2同，只是遍历前N个记录
dirty_map_limit(Fun, Tab, N) -> 
    dirty_map_limit_help(Fun, [], Tab, N, 1, dirty_first(Tab)).

dirty_map_limit_help(_Fun, Acc, _Tab, _N, _Count, '$end_of_table') ->
    lists:reverse(Acc); 
dirty_map_limit_help(_Fun, Acc, _Tab, N, Count, _Key) when Count > N -> 
    lists:reverse(Acc); 
dirty_map_limit_help(Fun, Acc, Tab, N, Count, Key) -> 
    Acc2 = [Fun(hd(dirty_read(Tab, Key))) | Acc],
    dirty_map_limit_help(Fun, Acc2, Tab, N, Count + 1, dirty_next(Tab, Key)).


%% dirty_map_limit/3同，不过是从表的末尾开始往前读N个元素
dirty_map_limit_from_end(Fun, Tab, N) -> 
    dirty_map_limit_from_end_help(Fun, [], Tab, N, 1, dirty_last(Tab)).

dirty_map_limit_from_end_help(_Fun, Acc, _Tab, _N, _Count, '$end_of_table') ->
    lists:reverse(Acc); 
dirty_map_limit_from_end_help(_Fun, Acc, _Tab, N, Count, _Key) when Count > N -> 
    lists:reverse(Acc); 
dirty_map_limit_from_end_help(Fun, Acc, Tab, N, Count, Key) -> 
    Acc2 = [Fun(hd(dirty_read(Tab, Key))) | Acc],
    dirty_map_limit_from_end_help(Fun, Acc2, Tab, N, Count + 1, dirty_prev(Tab, Key)).
