...
format_as_row_help([], [], _MaxHeadLen, _MaxValueLen, LineLen) -> 
	io:format("~s~n", [lists:duplicate(LineLen, "-")]);
format_as_row_help([Head | Rest1], [Value | Rest2], MaxHeadLen, MaxValueLen, LineLen) ->
	io:format("~s~n", [lists:duplicate(LineLen, "-")]),
	Len1 = integer_to_list(MaxHeadLen), 
	Len2 = integer_to_list(MaxValueLen), 
	io:format("~." ++ Len1 ++ "s | ~." ++ Len2 ++ "s ~n", [Head, Value]),
	format_as_row_help(Rest1, Rest2, MaxHeadLen, MaxValueLen, LineLen).
